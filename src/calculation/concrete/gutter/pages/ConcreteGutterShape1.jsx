...
const ConcreteGutterShape1 = () => {
  ...
  const [widthW1, setWidthW1] = useState("");
  const [widthW2, setWidthW2] = useState("");
  const [heightH1, setHeightH1] = useState("");
  const [heightH2, setHeightH2] = useState("");
  const [thicknessT1, setThicknessT1] = useState("");
  const [thicknessT2, setThicknessT2] = useState("");
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-6">
        <PageHeader
          title={t("concrete.gutter.gutterShape1", { defaultValue: "Concrete of Gutter Shape 1" })}
          showBackButton
          onBack={() => navigate(-1)}
        >
      ...
      <div>
        <div className="bg-[#F9F4EE] rounded-xl p-4">
          <div className="border-b border-gray-200 flex flex-col md:flex-row gap-6 pb-4 mb-4">
             <div>
                <img src={Gshape1Icon} alt="Gutter Shape 1" className="w-32 object-contain" />
             </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
                <NumberInput
                placeholder={t("concrete.byVolume.width_W1", { defaultValue: "Width - W1" })}
                value={widthW1}
                onChange={(e) => setWidthW1(e.target.value)}
                unit={unit === "metric" ? "m" : "ft"}
                />
                <NumberInput
                placeholder={t("concrete.byVolume.width_W2", { defaultValue: "Width - W2" })}
                value={widthW2}
                onChange={(e) => setWidthW2(e.target.value)}
                unit={unit === "metric" ? "m" : "ft"}
                />
            </div>
            <div className="grid grid-cols-2 gap-4">
                <NumberInput
                placeholder={t("concrete.byVolume.height_H1", { defaultValue: "Height - H1" })}
                value={heightH1}
                onChange={(e) => setHeightH1(e.target.value)}
                unit={unit === "metric" ? "m" : "ft"}
                />
                <NumberInput
                placeholder={t("concrete.byVolume.height_H2", { defaultValue: "Height - H2" })}
                value={heightH2}
                onChange={(e) => setHeightH2(e.target.value)}
                unit={unit === "metric" ? "m" : "ft"}
                />
            </div>
            <div className="grid grid-cols-2 gap-4">
                <NumberInput
                placeholder={t("concrete.byVolume.thickness_T1", { defaultValue: "Thickness - T1" })}
                value={thicknessT1}
                onChange={(e) => setThicknessT1(e.target.value)}
                unit={unit === "metric" ? "m" : "ft"}
                />
                <NumberInput
                placeholder={t("concrete.byVolume.thickness_T2", { defaultValue: "Thickness - T2" })}
                value={thicknessT2}
                onChange={(e) => setThicknessT2(e.target.value)}
                unit={unit === "metric" ? "m" : "ft"}
                />
            </div>
            <NumberInput
      ...
      <DownloadPDFModal
        ...
        defaultTitle={t("concrete.gutter.gutterShape1", { defaultValue: "Concrete of Gutter Shape 1" })}
      />
    </div>
  );
};

export default ConcreteGutterShape1;
