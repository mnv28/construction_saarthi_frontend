...
import TsimpleIcon from "../../../../assets/icons/St1.svg";
import { ROUTES_FLAT } from "../../../../constants/routes";

const ConcreteSimpleTube = () => {
  const { t } = useTranslation("calculation");
  const navigate = useNavigate();

  const [unit, setUnit] = useState("metric");
  const [outerDiameterD2, setOuterDiameterD2] = useState("");
  const [outerDiameterD1, setOuterDiameterD1] = useState("");
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-6">
        <PageHeader
          title={t("concrete.tube.simpleTube", { defaultValue: "Concrete of Simple Tube" })}
          showBackButton
          onBack={() => navigate(-1)}
        >
      ...
      <div>
        <div className="bg-[#F9F4EE] rounded-xl p-4">
          <div className="border-b border-gray-200 flex flex-col md:flex-row gap-6 pb-4 mb-4">
             <div>
                <img src={TsimpleIcon} alt="Simple Tube" className="w-32 object-contain" />
             </div>
          ...
          <div className="space-y-4">
            <NumberInput
              placeholder={t("concrete.byVolume.outerDiameter_D2", { defaultValue: "Outer Diameter - D2" })}
              value={outerDiameterD2}
              onChange={(e) => setOuterDiameterD2(e.target.value)}
              unit={unit === "metric" ? "m" : "ft"}
            />
            <NumberInput
              placeholder={t("concrete.byVolume.outerDiameter_D1", { defaultValue: "Outer Diameter - D1" })}
              value={outerDiameterD1}
              onChange={(e) => setOuterDiameterD1(e.target.value)}
              unit={unit === "metric" ? "m" : "ft"}
            />
            <NumberInput
                ...
                onClick={() =>
                  navigate(ROUTES_FLAT.CALCULATION_CONCRETE_BY_VOLUME_DETAILED, {
                    state: {
                      ...
                      rateOfConcrete,
                      unit,
                      pageTitle: t("concrete.tube.simpleTube", {
                        defaultValue: "Concrete of Simple Tube",
                      }),
                      ...results,
                    },
                  })
      ...
      <DownloadPDFModal
        ...
        defaultTitle={t("concrete.tube.simpleTube", { defaultValue: "Concrete of Simple Tube" })}
      />
    </div>
  );
};

export default ConcreteSimpleTube;
