...
// Import icons
import beamType7 from '../../../../assets/icons/beamType7.svg';
...
const BeamType7 = () => {
    ...
    // End Support Details
    const [endSupportD3, setEndSupportD3] = useState('');
    const [endSupportLengthCL, setEndSupportLengthCL] = useState('');
    ...
    const D3 = parseFloat(endSupportD3) || 0; // End Support Diameter
    const C = parseFloat(endSupportLengthCL) || 0; // End Support Length
    ...
    // Bottom Steel (D2)
    const bottomSteelWeightD2 = ((L + 540) * 0.002) * D2 * D2 * n / 162.28;

    // Curtail Length (C) - Using End Support Details
    // Formula from image: ((E+300) * 0.001) * D3 * D3 * n / 162.28
    // Assuming E corresponds to the input 'End support length CL' which we mapped to C
    const curtailSteelWeight = ((C + 300) * 0.001) * D3 * D3 * n / 162.28;
    ...
    const totalSteel = topSteelWeight + bottomSteelWeightD2 + curtailSteelWeight + stirrupsWeight;
    ...
    const calculationData = [
        ...
        { labelKey: 'steel.beam.endSupportD3', name: t('steel.beam.endSupportD3'), symbol: 'D3', value: `${endSupportD3} mm` },
        { labelKey: 'steel.beam.endSupportLengthCL', name: t('steel.beam.endSupportLengthCL'), symbol: 'C', value: `${endSupportLengthCL} mm` },
    ...
    const detailedOutputs = [
        ...
        {
            titleKey: 'steel.beam.curtailSteel',
            labelKey: 'steel.beam.curtailSteel',
            labelSuffix: '=',
            formula: '((E+300)X0.001)XD3XD3Xn/162.28',
            value: `${curtailSteelWeight.toFixed(3)} KG`
        },
        {
        ...
        },
        {
            ...
            labelSuffix: '=',
            formula: '(((L+540)X0.002)XD1XD1Xn/162.28)+(((L+540)X0.002)XD2XD2Xn/162.28)+(((E+300)X0.001)XD3XD3Xn/162.28)+(((2X(X-60)+2X(Y-60)+20XR))/1000X((L/S)+1)X(RXR/162.28)Xn)',
        ...
        },
        {
            ...
            labelSuffix: '=',
            formula: '((((L+540)X0.002)XD1XD1Xn/162.28)+(((L+540)X0.002)XD2XD2Xn/162.28)+(((E+300)X0.001)XD3XD3Xn/162.28)+(((2X(X-60)+2X(Y-60)+20XR))/1000X((L/S)+1)X(RXR/162.28)Xn)) * r',
            ...
            <div className="bg-[#F9F4EE] rounded-3xl p-4 sm:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    ...
                    <div className="flex items-center justify-center w-24 h-24 sm:w-38 sm:h-38">
                        <img src={beamType7} alt="Beam Diagram" className="w-full h-full object-contain" />
                    </div>
                ...
                <div className="space-y-4 pt-4">
                    ...
                    {/* End Support Details Section */}
                    <div className="space-y-2">
                        <h3 className="font-medium text-primary ml-1">{t('steel.beam.endSupportDetails')}</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4">
                            <InputField
                                unit="mm"
                                value={endSupportD3}
                                onChange={(e) => setEndSupportD3(e.target.value)}
                                placeholder={t('steel.beam.endSupportD3')}
                            />
                            <InputField
                                unit="mm"
                                value={endSupportLengthCL}
                                onChange={(e) => setEndSupportLengthCL(e.target.value)}
                                placeholder={t('steel.beam.endSupportLengthCL')}
                            />
                        </div>
                    </div>
            ...
            {showResult && (
                <div className="mt-10 animate-fade-in pb-10">
                        ...
                        <table className="w-full text-left border-collapse min-w-[320px]">
                            ...
                            <tbody className="divide-y divide-[#060C120A]">
                                {[
                                    ...
                                    { material: t('steel.beam.curtailSteel'), quantity: curtailSteelWeight.toFixed(3), unit: 'kg' },
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <Button
                            variant="primary"
                            onClick={() => navigate(ROUTES_FLAT.CALCULATION_BEAM_TYPE7_DETAILED, {
                                state: {
                                    calculationData,
