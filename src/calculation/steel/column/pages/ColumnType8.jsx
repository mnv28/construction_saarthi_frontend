...
const ColumnType8 = () => {
    ...
    // Vertical Steel (D1) - 4 Bars
    // Text formula: 4XD1XD1XHXn/162 (Using 162.28 for consistency unless strictly 162 requested, sticking to std)
    const weightD1 = (4 * D1 * D1 * H * nColumns) / 162.28;

    // Vertical Steel (D2) - 4 Bars
    // Text formula: 4XD2XD2XHXn/162
    const weightD2 = (4 * D2 * D2 * H * nColumns) / 162.28;

    // Stirrups (R1) - Outer Ring
    // Text Formula: ((2X(X-80)+2X(Y-80)+20XR1)/1000)X((1000XH/S)+1)X(R1XR1/162.28)Xn
    ...
    // Stirrups (R2) - Inner/Cross Link
    // Text Formula: (((X-80)+(Y-80)+40XR2)/1000)X((1000XH/S)+1)X(R2XR2/162)Xn
    // Interpretation: (X-80) and (Y-80) are lengths of the cross links + hooks/bends (40*R2)
    const cuttingLengthR2 = ((X - 80) + (Y - 80) + (40 * R2)) / 1000;
    const weightPerR2 = (R2 * R2) / 162.28;
    const stirrupWeightR2 = cuttingLengthR2 * numStirrups * weightPerR2 * nColumns;

    const totalSteel = weightD1 + weightD2 + stirrupWeightR1 + stirrupWeightR2;
    ...
    const calculationOutputs = [
        ...
        },
        {
            ...
            labelKey: 'steel.column.stirrups',
            labelSuffix: '(R2) =',
            formula: '(((X-80)+(Y-80)+40XR2)/1000)X((1000XH/S)+1)X(R2XR2/162)Xn',
        ...
        },
        {
            ...
            labelSuffix: ' =',
            formula: '(4XD1XD1XHXn/162)+(4XD2XD2XHXn/162)+(((2X(X-80)+2X(Y-80)+20XR1)/1000)X((1000XH/S)+1)X(R1XR1/162.28)Xn)+((((X-80)+(Y-80)+40XR2)/1000)X((1000XH/S)+1)X(R2XR2/162)Xn)',
        ...
        },
        {
            ...
            labelSuffix: ' =',
            formula: '((4XD1XD1XHXn/162)+(4XD2XD2XHXn/162)+(((2X(X-80)+2X(Y-80)+20XR1)/1000)X((1000XH/S)+1)X(R1XR1/162.28)Xn)+((((X-80)+(Y-80)+40XR2)/1000)X((1000XH/S)+1)X(R2XR2/162)Xn))XR',
            ...
            <div className="bg-[#F9F4EE] rounded-3xl p-4 sm:p-6">
                <div className="flex items-center gap-4 sm:gap-10 pb-6 border-b border-[#060C120A]">
                    ...
                    <div className="flex items-center justify-center w-24 h-24 sm:w-38 sm:h-38">
                        <img src={colType8} alt="Column Type 8 Diagram" className="w-full h-full object-contain" />
                    </div>
            ...
            {showResult && (
                <div className="mt-10 animate-fade-in pb-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <Button
                            variant="primary"
                            onClick={() => navigate(ROUTES_FLAT.CALCULATION_COLUMN_TYPE8_DETAILED, {
                                state: {
                                    calculationData,
