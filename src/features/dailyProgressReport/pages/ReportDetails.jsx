...
import { getDPRDetails } from '../api/dprApi';
...
export default function ReportDetails() {
  ...
  const projectName = location.state?.projectName || report?.project?.name || 'Project';
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        setIsLoading(true);
        const data = await getDPRDetails(reportId);
        setReport(data);
      } catch (error) {
  ...
  // Calculate title with auto-generation check
  const displayTitle = (report.title || report.description || report.metadata?.note || 'Untitled Report');
  const finalTitle = displayTitle.toLowerCase().includes('auto-generated by cron job') && report.project?.name
    ? report.project.name
    : displayTitle;
  ...
  const reportData = {
    title: finalTitle,
    workTime: {
      start: report.startTime || report.start_time || report.workTime?.start || '',
      end: report.endTime || report.end_time || report.workTime?.end || 'NA',
    },
    labour: {
      present: report.total_labour_present ?? report.presentLabours ?? 0,
      halfDay: report.total_labour_halfday ?? report.halfDayLabours ?? 0,
      absent: report.total_labour_absent ?? report.absentLabours ?? 0,
    },
    photos: report.photos || report.media || [],
    workProgress: {
      todaySummary: report.todaySummary || report.today_summary || report.workProgress?.todaySummary || (report.metadata?.note === 'Auto-generated by cron job' ? 'Auto-generated report' : 'N/A'),
      plannedWork: report.plannedWork || report.planned_work || report.workProgress?.plannedWork || 'N/A',
      actualWork: report.actualWork || report.actual_work || report.workProgress?.actualWork || 'N/A',
      delayReason: report.delayReason || report.delay_reason || report.workProgress?.delayReason || 'N/A',
    },
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-0">
      ...
      <PageHeader title={reportData.title}
        showBackButton={true}
        backTo={getRoute(ROUTES.DPR.PROJECT_REPORTS, { projectId })}
      >
